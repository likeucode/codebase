...
def preprocess_data(data_path):
	with open(data_path+'Img_feature_3.txt') as fea:
		fea_str=fea.read()
	fea_strlines=fea_str.splitlines()

	with open(data_path+'Img_label_3.txt') as f:
		raw_label=f.read()
	raw_label_strlines=raw_label.splitlines()

	feature=np.zeros((len(fea_strlines),512))
	...
	savemat("feature_vs_label.mat",{"feature":feature,"label":label})

	#return feature,label

def pca_train(data,n_components=128):
...
def joint_bayesian_train(trainingset, label, fold="./result/", threshold=1e-6, max_iter=100):
...
def lfw_pairlist(pair_path):
    with open(pair_path+"match_result_3.txt") as match:
        m=match.read()
    mlines=m.splitlines()

    with open(pair_path+"dismatch_result_3.txt") as dismatch:
        dism=dismatch.read()
    dismlines=dism.splitlines()

    fea_m=np.zeros((len(mlines),1024))
    label_m=np.ones(len(mlines))
    for i in range(fea_m.shape[0]):
        fea_m[i]=np.float32(mlines[i].split())

    fea_dism=np.zeros((len(dismlines),1024))
    ...
    savemat("lfw_pair.mat",{"fea_m":fea_m,"label_m":label_m,"fea_dism":fea_dism,"label_dism":label_dism})
...
def lfw_getratios(A,G,data_pair):
    ...
    return distance
def lfw_pca_test(intra,extra,result_fold='./lfw/'):
...
def lfw_test(lfw_pair_path,result_fold='./lfw/'):
    print "Executing test in lfw......"
...
def main():
	data_path="/home/ke/bozhang/"
	#pairlist_path="./data_pairlist.mat"
        lfw_pair_path="./lfw_pair.mat"
	preprocess_data(data_path)
	feature=loadmat('feature_vs_label.mat')['feature']
	label=loadmat('feature_vs_label.mat')['label']
	label=label.T
	label=label[:,0]
	...
	####train####
        pca_model=pca_train(feature,n_components=256)
        feature = pca_model.transform(feature)
        feature=preprocessing.scale(feature,axis=0)
        joint_bayesian_train(feature,label,threshold=1e-16, max_iter=500,fold="./lfw/")
        #lfw_test(lfw_pair_path,result_fold='./lfw/')
	####test####
        ...
        intra1 = pca_model.transform(intra[:,:512])
        intra2 = pca_model.transform(intra[:,512:])
        extra1 = pca_model.transform(extra[:,:512])
        extra2 = pca_model.transform(extra[:,512:])
...
if __name__ == '__main__':
	main()
	excute_performance("./lfw/result.pkl", -65, -40, 0.1)
